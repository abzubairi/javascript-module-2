...
let destinationNamesWithin500Kms = travelDestinations
  .filter((e) => e.distanceKms <= 500)
  .map((e) => e.destinationName); // Complete here

let destinationNameReachableByFerry = travelDestinations
  .filter((e) => e.transportations.includes("ferry"))
  .map((e) => e.destinationName); // Complete here

let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter((e) => e.transportations.includes("Train"))
  .map((e) => e.destinationName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
/*
